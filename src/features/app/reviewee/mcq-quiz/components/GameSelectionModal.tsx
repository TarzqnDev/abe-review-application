...
export default function GameSelectionModal(
  ...
  return (
    <QuizModalShell
      ...
      onBackdropMouseDown={gameSelectionModal.modalAccessibility.handleBackdropMouseDown}
    >
      ...
      <div className="space-y-5">
        <label className="block text-sm font-semibold text-primary-text">
          {gameSelectionModal.isPaesGame ? "Select PAES Subject" : "Select Area"}
          <span className="relative mt-2 block">
            <select
              value={gameSelectionModal.selectedOptionId}
              onChange={(event) => gameSelectionModal.setSelectedOptionId(event.target.value)}
              disabled={
            ...
            >
              {gameSelectionModal.isLoadingAreas ? (
                <option value="">
                  Loading {gameSelectionModal.isPaesGame ? "PAES subjects" : "areas"}...
                </option>
              ) : gameSelectionModal.selectionOptions.length === 0 ? (
                <option value="">
                  No {gameSelectionModal.isPaesGame ? "PAES subjects" : "areas"} available
                </option>
              ) : (
          ...
          <label className="block text-sm font-semibold text-primary-text">
            ...
            <span className="relative mt-2 block">
              <select
                ...
                onChange={(event) =>
                  gameSelectionModal.setDifficulty(event.target.value as typeof gameSelectionModal.difficulty)
                }
                disabled={gameSelectionModal.isPreparing}
              ...
              >
                {gameSelectionModal.quizDifficulties.map((difficultyOption) => (
