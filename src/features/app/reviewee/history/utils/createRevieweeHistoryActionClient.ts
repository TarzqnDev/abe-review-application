...
export const createRevieweeHistoryActionClient = async () => {
  ...
  if (userError || !user) {
    throw new Error("You must be logged in to view activity history");
  }
  ...
  if (roleError || !revieweeRole) {
    throw new Error("You are not authorized to view activity history");
  }
