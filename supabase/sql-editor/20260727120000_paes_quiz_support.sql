...
alter table public.question_sets
  drop constraint if exists question_sets_game_type_check,
  drop constraint if exists question_sets_difficulty_check,
  drop constraint if exists question_sets_subject_game_difficulty_key;

alter table public.question_sets
  add constraint question_sets_game_type_check check (
    game_type in (
      'Guess the Word',
      'AB-Solution',
      'Situationship',
  ...
  add constraint question_sets_difficulty_check check (
    (
  ...
  add constraint question_sets_subject_game_difficulty_key
    unique nulls not distinct (subject_id, game_type, difficulty);
...
alter table public.game_sessions
  ...
  add constraint game_sessions_difficulty_check check (
    (
      session_type = 'mcq_quiz'
      and game_type in ('Guess the Word', 'AB-Solution', 'Situationship')
      and difficulty in ('Easy', 'Medium', 'Hard')
    )
    or (
      session_type = 'mcq_quiz'
  ...
  add constraint game_sessions_paes_timer_check check (
    game_type <> 'PAES'
    or (
      session_type = 'mcq_quiz'
      and timer_seconds = 60
    )
  );
...
begin
  ...
  if selected_game_type = 'PAES' and selected_difficulty is null then
    return 60;
  end if;
...
declare
  ...
  selected_timer_seconds smallint := 60;
...
grant execute on function public.prepare_paes_quiz_session(bigint)
  to authenticated;
